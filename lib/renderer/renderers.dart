...
typedef Renderer =
  ...
  ///
  Widget Function({dynamic data, required FocusNode focusNode});
...
class Renderers {
  ...
  final Map<RendererType, Renderer> _renderers = {
    RendererType.markdown: ({dynamic data, required FocusNode focusNode}) => SelectionArea(focusNode: focusNode, child: Markdown(data: data))
  };

  Widget createWidget(RendererType rendererType, FocusNode focusNode, dynamic data) {
    final r = _renderers[rendererType];
    return r == null ? TextField(controller: TextEditingController(text: data),) : r(data: data, focusNode: focusNode);
  }
}
